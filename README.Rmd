...
The pipeline always starts by initializing a new ERAHUMED simulation, via:
...
layers involved in the simulation chain of the DSS, and is initially blank.

Next, we populate simulation layers, which is achieved through the set of
`compute_*()` functions:
...
We can extract and inspect simulation layers as illustrated in the following 
examples:

```{r}
hba(simulation)  # Extract the HBA simulation layer
hba(simulation) |> 
  get_layer_output() |>  # Get output data.frame
...
The order of simulation layers in the list above is the logical one. Each layer depends on the previous ones (referred to as "upstream"), and is a dependency of the subsequent ones (referred to as "downstream").
