...
`tests/` folder, and possibly on the scripts contained in the `data-raw/` and
`inst-raw/` folders. These four important directories are described in more 
detail below.
...
|
+--- inst/        * Additional files installed with the R package
|
+--- inst-raw/      Raw data and R scripts to generate the content of inst/
|
+--- man/         * R manual pages
|
...
This folder contains the scripts used to generate the content of the `inst/` 
folder, which in turn contains files that are installed together with the
`{erahumed}` R package. The workflow is manual and similar to the one described
in the previous section, to generate the content of `data/` from the scripts and
files contained in `data-raw/`.

It is instructive to inspect the content of these two folders at the time of 
writing:
...
The `parquet.R` script generates the file `parquet/albufera_hb_local.parquet`, 
which contains the precomputed result of `erahumed::albufera_hb_local()`. This 
is used to speed up the visualization of the Shiny app, which shows the results
of this computation by default. (We have set up a unit test that is triggered 
whenever `erahumed::albufera_hb_local()` needs to be updated, in which case the
script `parquet.R` should be manually triggered - this is suboptimal, but beside
the point here).
