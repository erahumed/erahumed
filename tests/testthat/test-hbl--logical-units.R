test_that("hbl_residence_time throws no errors on regular call", {
  ...
  expect_no_error(hbl_residence_time(volume, inflow))
})
...
test_that("hbl_residence_time 'k' argument controls level of smoothing", {
  ...
  expect_identical(
    hbl_residence_time(volume, inflow, k = k),
...
test_that("hbl_volume_change() succeeds with valid input", {
  expect_no_error(hbl_volume_change(rep(1, 10)))
})

test_that("hbl_volume_change() returns a double of the correct length", {
  len <- 10
  res <- hbl_volume_change(rep(1, len))
  expect_vector(res, ptype = double(), size = len)
})

test_that("hbl_volume_change(): last entry of output == fill_last", {
  fill <- 840
  res <- hbl_volume_change(rep(1, 10), fill_last = fill)
  expect_equal(res[length(res)], fill)
})

test_that("hbl_volume_change(): volume + differences == lagged volume", {
  volume <- c(4, 2, 6, 4, 6, 3, 7, 8, 4, 9, 1)
  volume_change <- hbl_volume_change(volume)
...
test_that("hbl_flow_balance(): succeeds with valid inputs", {
  expect_no_error(
    hbl_flow_balance(outflows = list(a = 1:10, b = 2:11),
...
test_that("hbl_flow_balance(): returns a dataframe of the correct length", {
  len <- 10
  res <- hbl_flow_balance(outflows = list(a = 1:len, b = (1:len) + 5),
...
test_that("hbl_flow_balance(): returns df has the required columns", {
  res <- hbl_flow_balance(outflows = list(a = 1:10, b = 2:11),
                          volume_change = rep(1, 10),
                          volume_change_petp = rep(0.5, 10)
                          )
...
test_that("hbl_flow_balance(): net balance checks", {
  ...
  res <- hbl_flow_balance(outflows = list(a = runif(len, 0, 1),
                                          b = runif(len, 0, 1)),
...
test_that("hbl_flow_balance(): outflow_recirculation > 0 requires zero inflow", {
  set.seed(840)
  len <- 1e3
  tol <- 1e-10
