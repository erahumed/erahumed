...
test_that("compute_inp(): no error with simplified outflows_df", {
  ...
  expect_no_error( compute_inp(erahumed_simulation(), outflows_df = outflows_df) )
})
...
test_that("compute_inp(): errors if provided with invalid input data", {
  ...
  expect_error(
    compute_inp(erahumed_simulation(), outflows_df = invalid_outflow_df),
    class = "compute_inp_argcheck_error"
  )
  expect_error(
    compute_inp(erahumed_simulation(), weather_df = invalid_weather_df),
    class = "compute_inp_argcheck_error"
  )
})
