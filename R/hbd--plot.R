...
plot.erahumed_hbd <- function(x, ditch = NULL, ...) {
  ...
  dygraphs::dygraph(data_ditch, main = paste("Time Series for ditch", ditch)) |>
    dygraphs::dySeries(name = "outflow_lake_m3", label = "Total Inflow", color = "black") |>
    dygraphs::dySeries(name = "inflow_clusters_m3", label = "Clusters Inflow", color = "blue") |>
