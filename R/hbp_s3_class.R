make_hbp <- function(df)
{
  make_hbp_argcheck(df)

  class(df) <- c("erahumed_hbp", class(df))
  attr(class(df), "package") <- "erahumed"
  return(df)
}


make_hbp_argcheck <- function(df) {
  tryCatch(
    {
      assert_data.frame(
        df,
        template = hbp_template_df(),
        extends = TRUE
        )
    ...
    error = function(e) {
      class(e) <- c("make_hbp_argcheck_error", class(e))
      stop(e)
    })
}

hbp_template_df <- function() {
  data.frame(
