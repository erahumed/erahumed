...
#' `albufera_hba()` and passed down to the `outflow` argument of
#' \link{hba}.
#'
#' @return Same as \link{hba}.
#'
#' @export
albufera_hba <- function(
...
{
  albufera_hba_datacheck(outflows_df, petp_df)
...
albufera_hba_datacheck <- function(outflows_df, petp_df) {
  tryCatch(
    {
    ...
    error = function(e) {
      class(e) <- c("albufera_hba_datacheck_error", class(e))
      stop(e)
    }
  )
